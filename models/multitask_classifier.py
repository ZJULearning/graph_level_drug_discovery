...
def get_loss_fn(final_loss):
  ...
  elif final_loss == 'focal_loss':

    def loss_fn(x, t, w, yita=1):
      xx = tf.nn.sigmoid(x)
      costs = (t * tf.pow(1-xx, yita * tf.ones_like(xx)) * tf.log(xx)) + (1 - t)  * tf.pow(xx, yita * tf.ones_like(xx)) * tf.log(1-xx)
...
class MultitaskGraphClassifier(Model):
  ...
  def __init__(self,
    ...
    super(MultitaskGraphClassifier, self).__init__(
        model_dir=None, verbose=verbose)
    ...
    if 1:
      ...
      self.lr = tf.train.exponential_decay(self.learning_rate, self.global_step, self.T, 0.1, staircase=True)
      # Set epsilon
      self.epsilon = 1e-7
      self.add_optimizer()
  ...
  def log(self,s,verbose):
      if verbose:
          print(s)
      f = open(self.logdir + '.log','a')
      f.write(s + '\n')
      f.close()
  ...
  def construct_feed_dict(self, X_b, y_b=None, w_b=None, training=1):
    ...
    training_dict = {self.training: training}
    ...
    feed_dict = merge_dicts([targets_dict, atoms_dict, training_dict])
    return feed_dict
  ...
  def add_training_loss(self, final_loss, logits):
    ...
    for task in range(self.n_tasks):
      ...
      task_loss = loss_fn(logits[task], one_hot_labels, task_weight_vector, yita = self.yita)
  ...
  def fit(self,
    ...
    self.log("%s Training for %d epochs" % (t, nb_epoch), self.verbose)

    # TODO(rbharath): Disabling saving for now to try to debug.
    for epoch in range(nb_epoch):
      #t = '['+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+']'
      #log("%s Starting epoch %d" % (t, epoch), self.verbose)
      total_loss = 0
      for batch_num, (X_b, y_b, w_b, ids_b) in enumerate(
          train_dataset.iterbatches(self.batch_size, pad_batches=self.pad_batches)):
        if (batch_num + 1) % log_every_N_batches == 0:
          t = '['+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+']'
          self.log("%s On batch %d: Loss: %5.5f; lr: %.5f;" % (t,batch_num + 1, total_loss / log_every_N_batches, self.lr.eval(session=self.sess)), self.verbose)
          total_loss = 0
        _, loss = self.sess.run(
            [self.train_op, self.loss_op], feed_dict=self.construct_feed_dict(X_b, y_b, w_b))
        total_loss += loss
      valid_scores = self.evaluate(valid_dataset, [metric], transformers)
      test_scores = self.evaluate(test_dataset, [metric], transformers)
      t = '['+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+']'
      self.log("%s, On epoch %d: valid_AUC: %.5f;test_AUC : %.5f;" % (t, epoch + 1, valid_scores['mean-roc_auc_score'],test_scores['mean-roc_auc_score']), self.verbose)
  ...
  def predict_on_batch(self, X):
    ...
    with self.sess.as_default():
      feed_dict = self.construct_feed_dict(X, training=0)
