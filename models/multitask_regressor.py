...
class MultitaskGraphRegressor(Model):
  ...
  def fit(self,
    ...
    for epoch in range(nb_epoch):
      ...
      valid_scores = self.evaluate(valid_dataset, [metric,rms,mae], transformers)
      test_scores = self.evaluate(test_dataset, [metric,rms,mae], transformers)
      t = '['+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+']'
      self.log("%s, On epoch %d: val_R: %.5f;val_RMS: %.5f;val_MAE: %.5f;test_R : %.5f;test_RMS : %.5f;test_MAE : %.5f;" % (t, epoch + 1, valid_scores['mean-pearson_r2_score'],valid_scores['mean-rms_score'],valid_scores['mean-mae_score'],test_scores['mean-pearson_r2_score'],test_scores['mean-rms_score'],test_scores['mean-mae_score']), self.verbose)
  ...
  def predict_proba(self, dataset, transformers=[], n_classes=1, **kwargs):
    """Wraps predict_proba to set batch_size/padding."""
    return super(MultitaskGraphRegressor, self).predict_proba(
